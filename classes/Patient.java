...
public class Patient extends JFrame implements ActionListener, MouseListener {
	...
	JButton b1,b2,b3,b4,b5, backBtn;
	JComboBox combo;
	JPanel panel;
	Color myColor;
	Font myFont;
	...
	public Patient (SecondPage sp)
	{
		super("Patient Section ");
		...
		patLabel = new JLabel(" Patient Section");
		patLabel.setBounds(200, 30, 200, 50);
		patLabel.setBackground(Color.BLUE);
		patLabel.setForeground(Color.WHITE);
		
		panel.add(patLabel);

		l1 = new JLabel(" Add New Patient");
		...
		b5 = new JButton("Make Payment");
		b5.setBounds(600, 300, 150, 50);
	...
	public void removePatient(String a)
	{
		int flag=0;
	...
	{    //acc=new HospitalAccount(a);
		for(int i=0;i<account.length;i++)
		{
		  if (account[i]!=null)
		   { 
			JOptionPane.showMessageDialog(null,i+" .Patient Name: "+account[i].getName());
				
			}
			}}

			public void searchPatient(String a)
	{
		int flag=0;
		...
		else {JOptionPane.showMessageDialog(null, "Patient not Found");}
	}
	 ...
	 public void actionPerformed(ActionEvent ae)
	{
		...
		{//showMessageDialog(null, "account inserted!");
			...
			addPatient(s1);   // insert method call
		    //insertTF.setText("");}
			//showMessageDialog(null, "account inserted!");}
		}
           ...
           else if(ae.getSource() == b3){showPatientInfo();}

            else if(ae.getSource() == b4){ searchPatient(s3);}

           else if(ae.getSource() == b5)
		{  
			PatientAccount pa= new PatientAccount(this);
			pa.setVisible(true);
